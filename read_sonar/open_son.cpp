...
int main(int argc, char** argv){
	...
	/// Setup Path ///
	std::string rootPath = "/home/microuuv/Documents/bvtsdk/";			// SDK path
	std::string dataPath = rootPath + "data/";					// .son path
	std::string mapperPath = rootPath + "colormaps/jet.cmap";	// Colormapper path
	std::string fileName;			// filename of .son
    nh.param<std::string>("file_name", fileName, "test1.son");
...
//        line(color_img,cv::Point(0,0),cv::Point(width/2,height),cv::Scalar(255,0,0),5,cv::LINE_AA);
//        line(color_img,cv::Point(width,0),cv::Point(width/2,height),cv::Scalar(255,0,0),5,cv::LINE_AA);
//
//        line(color_img,cv::Point(width/2/10,height/10),cv::Point(width/2/10+10,height/10),cv::Scalar(0,0,255),5,cv::LINE_AA);
//        line(color_img,cv::Point(width-width/2/10,height/10),cv::Point(width-width/2/10-10,height/10),cv::Scalar(0,0,255),5,cv::LINE_AA);
//
//        line(color_img,cv::Point(width/2,0),cv::Point(width/2,height),cv::Scalar(255,0,0),5,cv::LINE_AA);
//        line(color_img,cv::Point(width/2-5,height/10),cv::Point(width/2+5,height/10),cv::Scalar(0,0,255),5,cv::LINE_AA);
...
//        int udptest =sendto(sockfd,img_encode,img_encode.size(),0,(struct sockaddr*)&addr,len);
        ...
        std::string Img_Name = "/home/microuuv/phd_2021/Data/Sonar/0330/sonar" +std::to_string(temp_i)+".jpg";
        cv::imwrite(Img_Name, auto_img);
        temp_i += 1;
...
//		cv::resize(color_rth, color_rth, cv::Size(), 0.5, 0.5, interpolation) ;
		...
		/// ROS Message ///
		sensor_msgs::ImagePtr msg = cv_bridge::CvImage(std_msgs::Header(), "bgra8", color_img).toImageMsg();
		pub.publish(msg);
		ros::spinOnce();
		loop_rate.sleep();
